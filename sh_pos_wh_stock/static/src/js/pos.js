        ...
        class extends ProductScreen {
            ...
            async on_click_show_qty(event) {
                ...
                if (this.env.pos.config.sh_display_by == "location") {
                    var table_html = '<table width="100%" class="wh_qty"><tr><th width="70%" class="head_td">Location</th><th width="30%" class="head_td">Quantity</th></tr>';
                    var total_qty = 0;
                    ...
                    var table_html = '<table width="100%" class="wh_qty"><tr><th width="70%" class="head_td">Warehouse</th><th width="30%" class="head_td">Quantity</th></tr>';
                    var total_qty = 0;
                    $.each(quant_by_product_id, function (key, value) {
                        total_qty += parseInt(value);
    ...
    const WHStockProductsWidget = (ProductsWidget) =>
        class extends ProductsWidget {
            get productsToDisplay() {
                var self = this;
                if (this.searchWord !== "") {
                    return this.env.pos.db.search_product_in_category(this.selectedCategoryId, this.searchWord);
                } else {
                    var product_list = [];
